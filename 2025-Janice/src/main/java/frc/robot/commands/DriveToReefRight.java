...
public class DriveToReefRight extends DriveToPose {
...
public DriveToReefRight(Drive drive) {
    super(
        drive,
        () -> {
            ...
            Pose2d offsetPose = new Pose2d(
                new Translation2d(targetPose.getX() + Math.cos(targetPose.getRotation().getRadians()) * offsetDistanceBumper + Math.cos(targetPose.getRotation().getRadians() + Math.PI / 2) * Constants.DriveConstants.reefRightShift,
                targetPose.getY() + Math.sin(targetPose.getRotation().getRadians()) * offsetDistanceBumper + Math.sin(targetPose.getRotation().getRadians() + Math.PI / 2) * Constants.DriveConstants.reefRightShift),
